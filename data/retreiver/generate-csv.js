...
const generateCombinedCSV = async (area, fromDate, toDate) => {
  ...
  const header =
    "date,power_demand,load_shed,rainfall,mean_temperature,relativehumidity_2m,windspeed_10m,windspeed_100m,daytime_length,dewpoint_2m,cloudcover,soil_temperature_0_to_7cm\n"
  const rows = combinedRecords.map((record) => {
    return `${record.date},${record.demand},${record.loadShed},${record.rainfall},${record.mean_temperature},${record.relativehumidity_2m},${record.windspeed_10m},${record.windspeed_100m},${record.daytime_length},${record.dewpoint_2m},${record.cloudcover},${record.soil_temperature_0_to_7cm}\n`
  })
